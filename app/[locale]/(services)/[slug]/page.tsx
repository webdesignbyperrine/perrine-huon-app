...
function resolveSlug(slug: string, locale: string = 'fr'): PageType {
  if (getServiceBySlug(slug, locale as any)) return { kind: 'service', dataSlug: slug };
  if (slug.startsWith(LOCATION_PREFIX)) {
    const loc = slug.slice(LOCATION_PREFIX.length);
    if (getLocationBySlug(loc, locale as 'fr' | 'en' | 'es')) return { kind: 'location', dataSlug: loc };
  }
  if (slug.startsWith(PROFESSION_PREFIX)) {
    const prof = slug.slice(PROFESSION_PREFIX.length);
    if (getProfessionBySlug(prof, locale as any)) return { kind: 'profession', dataSlug: prof };
  }
  return null;
}
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  if (resolved.kind === 'service') {
    const s = getServiceBySlug(resolved.dataSlug, locale as any)!;
    return {
      title: s.metaTitle,
  ...
  if (resolved.kind === 'location') {
    const l = getLocationBySlug(resolved.dataSlug, locale as 'fr' | 'en' | 'es')!;
    return {
      title: l.metaTitle,
  ...
  const p = getProfessionBySlug(resolved.dataSlug, locale as any)!;
  return {
    title: p.metaTitle,
...
export default async function DynamicPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  if (resolved.kind === 'service') return <ServiceView slug={slug} dataSlug={resolved.dataSlug} locale={locale} />;
  if (resolved.kind === 'location') return <LocationView slug={slug} dataSlug={resolved.dataSlug} locale={locale} />;
  return <ProfessionView slug={slug} dataSlug={resolved.dataSlug} locale={locale} />;
}

// ---------------------------------------------------------------------------
// SERVICE VIEW
// ---------------------------------------------------------------------------
async function ServiceView({ slug, dataSlug, locale }: { slug: string; dataSlug: string; locale: string }) {
  const t = await getTranslations({ locale, namespace: 'services-pages.service' });
  const tBreadcrumb = await getTranslations({ locale, namespace: 'services-pages.breadcrumb' });
      ...
      <section className="relative grain-overlay pt-32 pb-20 bg-paper">
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            <Link href="/" className="inline-flex items-center gap-2 text-primary/50 hover:text-primary transition-colors mb-8 text-sm">{t('backToHome')}</Link>
      ...
      <section className="py-20 bg-paper-light grain-overlay">
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-bold text-primary text-center mb-14">{t('featuresTitle')}</h2>
      ...
      <section className="py-16 bg-paper-light grain-overlay">
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl mx-auto text-center bg-white/60 backdrop-blur-sm rounded-sketch-lg p-10 md:p-14 border-2 border-primary/10">
            <p className="text-sm uppercase tracking-widest text-primary/40 mb-3">{t('pricing.indicative')}</p>
            <p className="text-4xl md:text-5xl font-bold text-primary mb-4">{service.priceRange}</p>
            <p className="text-primary/60 mb-8 max-w-xl mx-auto">{t('pricing.uniqueProject')}</p>
            <Link href="/?openQualifier=true" className="btn-cta">{t('pricing.getQuote')}</Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-paper">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-bold text-primary text-center mb-14">{t('faqsTitle')}</h2>
      ...
      <section className="py-20 bg-primary grain-overlay">
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-white/70 text-lg mb-10 max-w-2xl mx-auto">{t('discuss')}</p>
...
// LOCATION VIEW
// ---------------------------------------------------------------------------
function LocationJsonLd({ name, slug, type, description }: { name: string; slug: string; type: string; description: string }) {
  const data = {
...
async function LocationView({ slug, dataSlug, locale }: { slug: string; dataSlug: string; locale: string }) {
  const t = await getTranslations({ locale, namespace: 'services-pages.location' });
  const tBreadcrumb = await getTranslations({ locale, namespace: 'services-pages.breadcrumb' });
  const location = getLocationBySlug(dataSlug, locale as 'fr' | 'en' | 'es')!;
  const typeLabel = t(`typeLabels.${location.type}` as any);
  const whyChoosePoints = t.raw('whyChoose.points') as Array<{ title: string; description: string }>;
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      <section className="relative bg-paper-light grain-overlay overflow-hidden">
        ...
        <div className="container mx-auto px-4 py-24 md:py-32 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-lg text-primary/60 max-w-2xl mx-auto">{t('specialist', { location: location.name })}</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 md:py-24 bg-paper-light grain-overlay">
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl sm:text-4xl font-bold text-primary mb-4 text-center">{t('businessTypes.title', { location: location.name })}</h2>
            <p className="text-primary/60 text-center mb-12 max-w-2xl mx-auto">{t('businessTypes.subtitle', { location: location.name })}</p>
      ...
      <section className="py-16 md:py-24 bg-paper">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl sm:text-4xl font-bold text-primary mb-4 text-center">{t('whyChoose.title', { location: location.name })}</h2>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            <div className="bg-white/60 backdrop-blur-sm rounded-sketch-lg p-10 md:p-14 border-2 border-primary/10">
              <h2 className="text-2xl sm:text-3xl font-bold text-primary mb-4">{t('keyStats')}</h2>
      ...
      <section className="py-16 md:py-24 bg-paper">
        <div className="container mx-auto px-4 text-center">
          <div className="max-w-3xl mx-auto bg-white/60 backdrop-blur-sm rounded-sketch-lg p-10 md:p-14 border-2 border-primary/10">
            <h2 className="text-3xl sm:text-4xl font-bold text-primary mb-4">{t('readyToCreate', { location: location.name })}</h2>
            <p className="text-primary/60 mb-8 max-w-xl mx-auto">{t('discuss', { location: location.name })}</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/?openQualifier=true" className="btn-cta">{t('startProject')}</Link>
              <Link href="/#rdv" className="btn-sketch">{t('bookCall')}</Link>
            </div>
          </div>
        </div>
      </section>
...
// PROFESSION VIEW
// ---------------------------------------------------------------------------
async function ProfessionView({ slug, dataSlug, locale }: { slug: string; dataSlug: string; locale: string }) {
  const t = await getTranslations({ locale, namespace: 'services-pages.profession' });
  const tBreadcrumb = await getTranslations({ locale, namespace: 'services-pages.breadcrumb' });
  const profession = getProfessionBySlug(dataSlug, locale as any)!;
  ...
  return (
    <div className="min-h-screen bg-paper-light grain-overlay">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-lg text-primary/60 max-w-2xl mx-auto">{t('customSite')}</p>
      ...
      <section className="py-20 bg-paper-light">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl font-bold text-primary mb-4">{t('keyFeatures', { profession: profession.name.toLowerCase() })}</h2>
          </div>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto bg-accent/5 border-2 border-accent/20 rounded-sketch-lg p-8 sm:p-12">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-primary mb-4">{t('regulations')}</h2>
      ...
      <section className="py-20 bg-paper-light">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16"><h2 className="text-3xl sm:text-4xl font-bold text-primary mb-4">{t('faqs')}</h2></div>
      ...
      <section className="py-20 bg-paper">
        <div className="container mx-auto px-4 text-center">
          <div className="max-w-3xl mx-auto bg-paper-light border-2 border-primary/10 rounded-sketch-lg p-12 hover:border-primary/20 transition-all duration-300">
            <h2 className="text-3xl font-bold mb-4 text-primary">{t('readyToCreate')}</h2>
            <p className="text-primary/60 mb-8 max-w-xl mx-auto">{t('discuss')}</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/?openQualifier=true" className="btn-cta">{t('startProject')}</Link>
              <Link href="/#rdv" className="btn-sketch">{t('bookCall')}</Link>
            </div>
